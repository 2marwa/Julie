...
def check_inputs ():
    ...
    if (entry_receipt_number.get()) :
        if (entry_receipt_number.get()) > 1 or  (receipt_number.get()) > 500:
            input_check = 1
...
def append_name ():
    #generating a random reciept number
    receipt_number = random.randint(1000000, 9999999)
    #append each item to its own area of the list
    hire_details.append([entry_name.get(), entry_item.get(), entry_hires.get(), receipt_number.get()])
    hire_details.append([receipt_number])
    #clear the boxes
...
def setup_buttons():
    ...
    Button(main_window, text="Print Details", command=print_hire_details, width=10).grid(column=4, row=1, sticky=E)
